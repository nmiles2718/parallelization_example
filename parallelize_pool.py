...
def parallel(data_to_process, use_thread, flist = None):
    ...
    if not flist:
        ...
        with mp.Pool(os.cpu_count()) as pool:
            sources = pool.starmap(analyze, zip(data_to_process, [None]*(len(data_to_process))))
        end_time = time.time()
        total = (end_time - start_time)/60
    else:
        print('Using data from fits file')
        start_time = time.time()
        with mp.Pool(os.cpu_count()) as pool:
            sources = pool.starmap(analyze, zip([None]*(len(flist)), flist))
        end_time = time.time()
        total = (end_time - start_time)/60
